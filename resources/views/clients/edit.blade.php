<x-app-layout>
    ...
    <x-slot name="subtitle">Update the profile and important dates for {{ $client->name }}.</x-slot>
        ...
        <div class="space-y-6 lg:space-y-8">
            ...
            <x-section-card title="Contact Information">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <x-text-input id="company_name" name="company_name" type="text" class="mt-1 block w-full" :value="old('company_name', $client->company_name)" />
                        <x-input-error :messages="$errors->get('company_name')" class="mt-2" />
                    </div>

                    <div class="sm:col-span-2">
                        <x-input-label for="address" value="Physical Address" />
                        <textarea id="address" name="address" rows="3" class="mt-1 block w-full border-slate-300 focus:border-primary-500 focus:ring-primary-500 rounded-lg shadow-sm text-slate-900 sm:text-sm transition-colors duration-200">{{ old('address', $client->address) }}</textarea>
            ...
            <x-section-card title="Important Dates">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="anniversary_date" name="anniversary_date" type="date" class="mt-1 block w-full" :value="old('anniversary_date', optional($client->anniversary_date)->format('Y-m-d'))" />
                        <x-input-error :messages="$errors->get('anniversary_date')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="premium_due_date" value="Premium Due Date" />
                        <x-text-input id="premium_due_date" name="premium_due_date" type="date" class="mt-1 block w-full" :value="old('premium_due_date', optional($client->premium_due_date)->format('Y-m-d'))" />
            ...
            <x-section-card title="Groups (Tags)">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    @php
                        $clientGroups = $client->groups->pluck('id')->toArray();
                    @endphp
                    @forelse($groups as $group)
                        <label class="flex items-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="checkbox" name="groups[]" value="{{ $group->id }}" class="rounded border-slate-300 text-primary-600 shadow-sm focus:ring-primary-500" {{ in_array($group->id, old('groups', $clientGroups)) ? 'checked' : '' }}>
            ...
            <x-section-card title="Policy & Additional Details">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <x-text-input id="policy_name" name="policy_name" type="text" class="mt-1 block w-full" :value="old('policy_name', $client->policy_name)" placeholder="e.g. Health Shield Gold" />
                        <x-input-error :messages="$errors->get('policy_name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="policy_number" value="Policy Number" />
                        <x-text-input id="policy_number" name="policy_number" type="text" class="mt-1 block w-full" :value="old('policy_number', $client->policy_number)" />
